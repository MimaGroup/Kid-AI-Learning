...
export default async function FriendsPage() {
  ...
  try {
    ...
    const secretKey = profile?.secret_key || "USTVARJANJE ..."
    const userName = profile?.display_name || profile?.email || user.email || "Uporabnik"
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 flex items-center justify-center p-4">
        <div className="bg-red-500/10 border-2 border-red-500 rounded-lg p-6 max-w-md">
          <h2 className="text-white text-xl font-bold mb-2">Napaka pri nalaganju prijateljev</h2>
          <p className="text-white/80">{error instanceof Error ? error.message : "Prišlo je do neznane napake"}</p>
        </div>
      </div>
    )
  }
